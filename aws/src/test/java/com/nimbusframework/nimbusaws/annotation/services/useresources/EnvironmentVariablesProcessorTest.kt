...
class EnvironmentVariablesProcessorTest : AnnotationSpec() {
    ...
    fun correctlySetsVariableFromString() {
        compileState.compileObjects {
            ...
            HttpFunctionResourceCreator(cfDocuments, nimbusState, it, setOf(), mockk(relaxed = true)).handleElement(elements.getTypeElement("handlers.UsesEnvironmentVariableHandler").enclosedElements[1], FunctionEnvironmentService(cfDocuments, nimbusState))
            HttpFunctionResourceCreator(cfDocuments, nimbusState, it, setOf(), mockk(relaxed = true)).handleElement(elements.getTypeElement("handlers.UsesEnvironmentVariableHandler").enclosedElements[2], FunctionEnvironmentService(cfDocuments, nimbusState))
